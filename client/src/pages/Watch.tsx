...
import { ArrowRight, CheckCircle2, Clock3, Gift, Info, Play, RefreshCw, ShieldCheck, TimerReset, XCircle } from "lucide-react";
    ...
    <div className="page-stack">
      ...
      <section className="reward-layout">
        <div className="reward-card glass-card">
          ...
          <div className="reward-main"><div className="reward-copy"><h2>Watch an optional<br /><strong>eligible ad</strong> to unlock 5 points.</h2><p>Points are added only after provider verification. A timer alone never changes your balance.</p><div className="reward-rules"><span><CheckCircle2 size={15} /> one-time reward session</span><span><CheckCircle2 size={15} /> duplicate checks applied</span><span><CheckCircle2 size={15} /> verification required</span></div></div><div className="timer-stack"><div className={`timer-ring ${hasStarted ? "timer-ring-active" : ""}`} style={ringStyle}><div className="timer-inner">{secondsLeft}<small>sec</small></div></div><span className="timer-label">{hasStarted ? "session active" : "20 seconds"}</span></div></div>
          <div className="ad-embed-shell"><div className="ad-embed-topline"><span><span className="ad-dot" /> live ad placement</span><span className="embed-lock">A-ADS / {PLACEMENT_ID}</span></div><div className="ad-embed-content"><iframe title="A-ADS rewarded placement 2454518" data-aa="2454518" src="//ad.a-ads.com/2454518?size=320x100" loading="lazy" referrerPolicy="no-referrer-when-downgrade" /><small>Complete the visible ad experience, then wait for a trusted verification signal before points can be added.</small></div></div>
