...
export default function Watch() {
  ...
  useEffect(() => {
    if (hasStarted && secondsLeft === 0) {
      setNotice("pending");
      setSession((current) => current ? { ...current, status: "PENDING_VERIFICATION" } : current);
    }
  }, [hasStarted, secondsLeft]);
  ...
  function startRewardSession() {
    const next = createRewardSession();
    setSession({ ...next, status: "STARTED" });
    ...
    <div className="page-stack">
      ...
      <section className="reward-layout">
        <div className="reward-card glass-card">
          <div className="reward-card-header"><span className="reward-badge"><Gift size={14} /> reward available</span><span className="network-label">A-ADS / {PLACEMENT_ID}</span></div>
          ...
          <div className="ad-embed-shell"><div className="ad-embed-topline"><span><span className="ad-dot" /> secure ad placement</span><span className="embed-lock"><LockKeyhole size={12} /> sandboxed</span></div><div className="ad-embed-content"><div className="ad-symbol"><Play size={19} fill="currentColor" /></div><strong>A-ADS rewarded placement</strong><span>Placement {PLACEMENT_ID} · no close control</span><small>Network inventory is checked before a session can be verified.</small></div></div>
          <div className="reward-card-footer"><div className="session-state">{session ? <><span className="state-dot state-dot-live" /> {session.status.replaceAll("_", " ").toLowerCase()}</> : <><Clock3 size={14} /> ready to start</>}</div>{hasStarted ? <button className="secondary-button" onClick={resetSession}><RefreshCw size={15} /> reset</button> : <button className="primary-button" onClick={startRewardSession}><Play size={15} fill="currentColor" /> Watch Ad</button>}</div>
        </div>
      ...
      {notice === "pending" && <div className="status-banner status-pending glass-card"><TimerReset size={18} /><div><strong>Verification pending.</strong><span>The network must confirm this session before points can be added.</span></div><button onClick={() => { setNotice("unavailable"); setSession((current) => current ? { ...current, status: "REJECTED" } : current); }}>Dismiss</button></div>}
